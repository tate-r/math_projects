...
To plot a vector, simply do the following:
	
		>>> v=Vector([3,4])
		>>> plotter.plot(v)
		
A matplotlib window will appear with a colored line marking the vector.
	
Bear in mind that the "plot" function can take mulitple inputs, and hence 
is useful for seeing the effects of a matrix on vectors:
	
		>>> plotter.plot(v1 , m*v1 , v2 , m*v2)
